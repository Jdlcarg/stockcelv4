...
export class CashAutomationService {
  ...
  private async checkScheduledOperations() {
    try {
      ...
      for (const client of clients) {
        if (!client.isActive) {
          console.log(`🕐 [CASH-AUTO] Skipping inactive client ${client.id}`);
          continue;
        }

        console.log(`🕐 [CASH-AUTO] Processing client ${client.id}`);
  ...
  async executeAutoOpen(clientId: number) {
    try {
  ...
  async executeAutoClose(clientId: number) {
    try {
